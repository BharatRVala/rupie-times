...
import { FaArrowLeft, FaColumns, FaBars, FaTimes, FaFacebookF, FaInstagram, FaYoutube, FaStar, FaRegStar, FaRegEdit, FaRegTrashAlt } from 'react-icons/fa';
import { FiPlus } from 'react-icons/fi';
import GlobalLoader from '@/app/components/GlobalLoader';
import sharpIcon from "@/app/assets/sharp.svg";

// Exact replica of ProductArticleViewPage but adapted for Free Articles
export default function ArticleViewPage() {
    // Unpack params
    const params = useParams();
    const articleId = params.id; // In Free View, [id] is articleId
    const router = useRouter();

    // Configuration
    // For free articles, we don't have a product ID base path generally, but we can stick to articles list
    const basePath = `/admin-dashboard/articles`;
    // Image base path for free articles? 
    // If images are stored similarly, we might need a general endpoint or just rely on absolute URLs if provided.
    // We'll use a generic safe image helper.
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true);
                // Use generic article fetch
                const response = await fetch(`/api/admin/articles/${articleId}?t=${Date.now()}`, {
                    cache: 'no-store',
                    headers: {
                        'Pragma': 'no-cache',
    ...
    // Fetch Sidebar (Recent Articles for context)
    useEffect(() => {
        const fetchSidebarArticles = async () => {
            try {
                setLoadingSidebar(true);
                // Fetch recent free articles
                const res = await fetch(`/api/admin/articles?limit=10&minimal=true`);
    ...
    const renderBlock = (block, index) => {
        ...
        switch (bType) {
            ...
            case 'footer':
                const foot = (typeof parsedContent === 'object') ? parsedContent : {};
                return (
                ...
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 relative transition-all duration-300 ease-in-out">
                    <div className={`w-full transition-all duration-300 ease-in-out ${isDesktopSidebarOpen ? 'lg:w-[60%]' : 'lg:w-full'}`}>
                        <div className="relative w-full h-[300px] md:h-[400px] rounded-xl overflow-hidden mb-8">
                            <Image src={getSafeImageUrl(article.featuredImage || article.image, 'featured')} alt={article.mainHeading} fill className="object-cover" onError={() => handleImageError('featured')} />
                        </div>
                        <div className="flex justify-between items-start mb-4">
                            <h1 className="text-3xl md:text-4xl font-bold text-black leading-tight">{article.mainHeading}</h1>
                            {/* <button className="p-2 text-[#C0934B] opacity-50 cursor-not-allowed" title="Actions disabled in preview"><FaRegStar className="w-6 h-6" /></button> */}
                        </div>
                        ...
                        <div className="flex justify-center mb-8 relative z-10 px-2 border-b border-gray-200 pb-4">
                            <button
                                onClick={() => router.push(`/admin-dashboard/articles/sections/${articleId}`)}
                                className="flex items-center gap-2 px-4 py-2 bg-[#C0934B] text-white rounded-lg shadow-md hover:bg-[#A07A3E] transition-all duration-300 font-bold"
                            >
                        ...
                        <div className="space-y-8">
                            ...
                            {sections.length > 0 && (
                                <div className="flex justify-center mb-4 -mt-4 relative z-10 px-2">
                                    <button
                                        onClick={() => router.push(`/admin-dashboard/articles/sections/${articleId}?position=${sections.length}`)}
                                        ...
                                        <div key={section._id || index}>
                                            <div className={sectionStyle} id={`section-${section._id}`}>
                                                <div className="flex items-start mb-4 w-full">
                                                    <h3 className="text-xl font-bold text-black flex-grow">{section.heading}</h3>
                                                    <div className="flex items-center gap-2 ml-auto">
                                                        <button
                                                            className="p-2 text-blue-600 hover:bg-blue-50 rounded-full transition-colors"
                                                            title="Edit Section"
                                                            onClick={() => router.push(`/admin-dashboard/articles/sections/${articleId}/${section._id}/edit`)}
                                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={async () => {
                                                                if (confirm('Are you sure you want to delete this section?')) {
                                                                    ...
                                                                    try {
                                                                        const res = await fetch(`/api/admin/articles/${articleId}/sections/${section._id}`, {
                                                                            method: 'DELETE'
                                                                        });
                                                                        const data = await res.json();
                                                ...
                                                <div className="space-y-6">
                                                    {section.contentBlocks?.length ?
                                                        section.contentBlocks
                                                            ...
                                                            // .filter(block => block.blockType !== 'footer') // REMOVED FILTER to allow footer editing
                                            ...
                                            <div className="flex justify-center mb-8 -mt-4 relative z-10 px-2">
                                                <button
                                                    onClick={() => router.push(`/admin-dashboard/articles/sections/${articleId}?position=${trueIndex}`)}
                                                    className="w-8 h-8 flex items-center justify-center bg-[#C0934B] text-white rounded-[4px] shadow-md hover:bg-[#A07A3E] transition-all duration-300"
                            ...
                            {/* Global Footer (Removed separate rendering, now handled in sections) */}
                        </div>
                    </div>
                    ...
                    <div className={`fixed inset-x-0 bottom-0 top-16 sm:top-18 z-40 bg-[#F5F5F7] lg:bg-transparent lg:static lg:h-auto transform transition-all duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'} ${isDesktopSidebarOpen ? 'lg:w-[40%] lg:opacity-100 lg:visible' : 'lg:w-0 lg:opacity-0 lg:invisible lg:overflow-hidden'}`}>
                        <div className="h-full overflow-y-auto p-6 lg:p-0 lg:pl-8">
                            <h2 className="text-2xl font-bold mb-6 hidden lg:block">Latest Articles</h2>
                            {loadingSidebar ? (
                                    ...
                                    {sidebarArticles.map((item) => (
                                        <div key={item._id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow backdrop-blur supports-[backdrop-filter]:bg-white/10 bg-white/10">
                                            ...
                                            <div className="flex justify-between items-center mt-2">
                                                ...
                                                <Link href={`/admin-dashboard/articles/view/${item._id}`} className="text-xs font-bold text-[#00301F] flex items-center hover:underline">Preview <FaArrowLeft className="ml-1 rotate-180 w-3 h-3" /></Link>
                                            </div>
                                        </div>
                                    ))}
                                </div>
