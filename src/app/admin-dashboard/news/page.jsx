...
export default function NewsManagementPage() {
    ...
    const { pageHeader, searchPlaceholder, filterOptions, addButtonText } = adminNewsData;
    ...
    const fetchNews = async () => {
        ...
        try {
            ...
            if (data.success) {
                const mappedNews = data.articles.map(news => ({
                    _id: news._id,
    ...
    const filteredNews = useMemo(() => {
        ...
        return allNews.filter(news => {
            ...
            return (
                (news.mainHeading || "").toLowerCase().includes(lowerQuery) ||
                ...
                (news.createdBy || "").toLowerCase().includes(lowerQuery)
            );
        });
    }, [allNews, statusFilter, searchQuery]);

    // Pagination Logic
    const visibleNews = useMemo(() => {
        if (itemsPerPage === 'All') return filteredNews;
        return filteredNews.slice(0, itemsPerPage);
    }, [filteredNews, itemsPerPage]);

    // Handle toggle switch (Active/Inactive)
    const handleToggle = async (id, currentStatus) => {
        const newsItem = allNews.find(n => n._id === id);
        if (!newsItem) return;

        const newStatus = !currentStatus;

        // Optimistic update
        setAllNews(prev => prev.map(n => n._id === id ? { ...n, isActive: newStatus, status: newStatus ? "active" : "inactive" } : n));
                                ...
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-700">{news.category}</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <span className="font-medium">By</span>
                                                <span>{news.author || news.createdBy || "Admin"}</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                    ...
                                    <div className="flex flex-col items-end gap-3 ml-4">
                                        <div className="flex items-center gap-3">
                                            {news.isImportant && (
                                                <span className="px-3 py-1 bg-[#E8D4FF] text-[#6B21A8] text-xs font-semibold rounded-full">
                                                    Important
                                                </span>
                                            )}
                                            {/* Toggle Switch */}
                                            <button
                                                onClick={() => handleToggle(news._id, news.isActive)}
                                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#C0934B] focus:ring-offset-2 ${news.isActive ? "bg-green-500" : "bg-gray-300"
                                                    }`}
                                                title={news.isActive ? "Deactivate" : "Activate"}
                                            >
                                                <span
                                                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${news.isActive ? "translate-x-6" : "translate-x-1"
                                                        }`}
                                                />
                                            </button>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            {/* Plus Button - Manage Sections */}
                                            <button
                                                onClick={() => router.push(`/admin-dashboard/news/sections/${news._id}`)}
                                            ...
                                            {/* View Button */}
                                            <button
                                                onClick={() => router.push(`/admin-dashboard/news/${news._id}`)}
                                                className="w-8 h-8 flex items-center justify-center bg-[#C0934B] hover:bg-[#a17a3e] rounded-md transition-colors shadow-sm"
                                                title="View"
                                            >
                                                <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            {/* Edit Button */}
                                            <button
                                                onClick={() => router.push(`/admin-dashboard/news/edit/${news._id}`)}
                                                className="w-8 h-8 flex items-center justify-center bg-[#C0934B] hover:bg-[#a17a3e] rounded-md transition-colors shadow-sm"
                                                title="Edit"
                                            >
                                                <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
