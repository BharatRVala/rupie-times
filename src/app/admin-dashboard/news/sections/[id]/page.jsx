...
import { FaTrash, FaFacebookF, FaInstagram, FaYoutube, FaArrowLeft } from 'react-icons/fa';
...
function NewsSectionsPage() {
    ...
    const uploadImageFile = async (file) => {
        ...
        const uploadUrl = '/api/admin/news/upload';
    ...
    const handleAddSection = async () => {
        ...
        try {
            ...
            // Use the News Sections API with position support
            const response = await fetch(`/api/admin/news/${newsId}/sections`, {
                method: 'POST',
    ...
    return (
        <div className="max-w-7xl mx-auto pb-10">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <button
                    onClick={handleBack}
                    className="px-4 py-2 bg-white border border-[#C0934B] text-[#C0934B] rounded-lg hover:bg-[#C0934B] hover:text-white transition-colors flex items-center gap-2"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-6 relative">
                    ...
                    <h2 className="text-xl font-bold text-gray-900 mb-4">Add New Section {position !== undefined ? `at Position ${position + 1}` : ''}</h2>
                                    ...
                                    {showBlockTypeMenu && (
                                        <div className="absolute bottom-12 right-0 bg-white border border-gray-200 rounded-lg shadow-lg p-2 w-48 z-10">
                                            ...
                                            {blockTypes.map(bt => {
                                                ...
                                                const footerExists = newsItem?.sections?.some(s => s.contentBlocks?.some(b => b.blockType === 'footer' || b.type === 'footer')) || contentBlocks.some(b => b.type === 'footer');
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        title={disabled ? "Only one footer allowed" : ""}
                                                    >
                                                        <span className="font-bold w-6 text-center">{bt.icon}</span>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-end gap-4">
                            <div className="space-y-1">
                                <label className="block text-sm text-gray-600">Rows</label>
                            ...
                            <div className="space-y-1">
                                <label className="block text-sm text-gray-600">Cols</label>
                ...
                return (
                    <div className="space-y-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <div className="space-y-2">
                            ...
                            <input value={fData.author || ''} onChange={e => uFooter('author', e.target.value)} placeholder="e.g. Rupie Times Admin" className="w-full border border-gray-300 rounded-lg p-2.5 bg-white focus:ring-2 focus:ring-[#C0934B] outline-none" />
                        </div>
...
function ContentBlockPreview({ block }) {
    ...
    if (block.type === 'heading') return <h4 className="text-xl font-bold">{parseStyledText(block.content)}</h4>;
    if (block.type === 'paragraph') return <div className="text-gray-700 leading-relaxed mb-4">{parseStyledText(block.content)}</div>;
    if (block.type === 'quote') return <blockquote className="border-l-4 border-[#C0934B] pl-4 italic text-gray-600 my-4">{parseStyledText(block.content)}</blockquote>;
    ...
    if (block.type === 'image') {
        ...
        if (!url) return null;
        return (
            <div className="relative rounded-xl overflow-hidden shadow-md my-6">
                <img src={url} className="w-full h-auto object-cover" alt={caption || 'Preview'} />
                {caption && (
    ...
    if (block.type === 'list') {
                    ...
                    {items.map((item, i) => (
                        <li key={i} className="flex items-start gap-2">
                            {data.customImage ? (
                                ...
                                <span className="text-[#C0934B] font-bold mt-1">{data.customSymbol || '•'}</span>
                            )}
                            <span className="text-gray-700">{parseStyledText(item)}</span>
                        </li>
                    ))}
                </ul>
            );
        ...
        return <ul style={{ listStyleType: listStyleMap[data.style] || data.style || 'disc' }} className="pl-6 space-y-2 my-4 text-gray-700 list-outside">{items.map((item, i) => <li key={i}>{parseStyledText(item)}</li>)}</ul>;
    }

    if (block.type === 'link') {
        const linkData = typeof block.content === 'object' ? block.content : { url: block.content, text: 'Review Link', target: 'same' };
        return <a href={linkData.url} target={linkData.target === 'new' ? '_blank' : '_self'} className="text-[#C0934B] hover:underline block my-4 font-bold text-lg">{linkData.text || linkData.url}</a>;
    }
            ...
            <div className="overflow-x-auto my-6 rounded-lg shadow-sm border border-gray-200">
                <table className="min-w-full divide-y divide-gray-200">
                    <tbody className="divide-y divide-gray-200">
                        {t.data.map((r, i) => (
                            <tr key={i} className={i === 0 ? 'bg-[#D2B48C]' : i % 2 === 0 ? 'bg-[#FDF5E6]' : 'bg-white'}>
                                {r.map((c, j) => <td key={j} className={`px-6 py-4 text-sm ${i === 0 ? 'text-black font-bold' : 'text-gray-700'}`}>{parseStyledText(c)}</td>)}
                            </tr>
                        ))}
                    </tbody>
                </table>
...
export default function NewsSectionsPageWrapper() {
    return (
