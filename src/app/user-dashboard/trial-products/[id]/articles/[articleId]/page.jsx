...
import { FaArrowLeft, FaRegStar, FaStar, FaBars, FaTimes, FaInbox, FaPrint, FaColumns, FaFacebookF, FaInstagram, FaYoutube } from 'react-icons/fa';
...
export default function TrialSingleArticlePage({ params }) {
    const handlePrint = () => {
        window.print();
    };
    ...
    // UI State
    const isStarred = isFavorite(paramArticleId);
    ...
    const basePath = `/user-dashboard/trial-products`;

    // --- Data Fetching ---

    useEffect(() => {
        const fetchContextAndArticle = async () => {
            try {
                // 1. Fetch Sidebar Articles (Trial List)
                setLoadingSidebar(true);
                const listRes = await fetch(`/api/user/trial/products/${paramProductId}`);
                const listData = await listRes.json();

                if (listData.success) {
                    const filtered = listData.articles.filter(a => a._id !== paramArticleId);
                    setSidebarArticles(filtered || []);
                }

                // 2. Fetch Specific Article Details (With Progress)
                // We use the Subscription API which now supports Trial Access
                setLoading(true);
                const detailRes = await fetch(`/api/user/products/${paramProductId}/articles/${paramArticleId}`);
                const detailData = await detailRes.json();

                if (detailData.success) {
                    const sortedArticle = {
                        ...detailData.article,
                        sections: detailData.article.sections || []
                    };
                    setArticle(sortedArticle);
                    setSections(sortedArticle.sections || []);
                    setReadingProgress(detailData.readingProgress);
                    setLoading(false);
                } else {
                    setError(detailData.error || 'Failed to load article details');
                    setLoading(false);
                }
    ...
    useEffect(() => {
        if (readingProgress && sections.length > 0) {
            ...
            const validReadSections = readingProgress.readSections?.filter(rs =>
                validSections.some(s => s._id?.toString() === rs.sectionId?.toString())
            ) || [];
    ...
    const markSectionAsRead = async (sectionId, timeSpent = 30) => {
        // Ensure sectionId is string
        const targetId = sectionId.toString();
        const isCurrentlyRead = isSectionRead(targetId);

        console.log(`[MarkAsRead] Toggling section ${targetId} | Currently Read: ${isCurrentlyRead}`);

        // Optimistic Update
        const previousProgress = { ...readingProgress };
        let newReadSections = [];

        if (readingProgress?.readSections) {
            newReadSections = [...readingProgress.readSections];
        }

        if (isCurrentlyRead) {
            // Unmark
            newReadSections = newReadSections.filter(s => {
                const sId = s.sectionId || s._id;
                return sId?.toString() !== targetId;
            });
        } else {
            // Mark
            newReadSections.push({ sectionId: targetId, readAt: new Date() });
        }

        setReadingProgress(prev => ({ ...prev, readSections: newReadSections }));
        setMarkingRead(prev => ({ ...prev, [targetId]: true }));

        try {
            const response = await fetch(`/api/user/products/${paramProductId}/articles/${article._id}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ sectionId: targetId, timeSpent, unread: isCurrentlyRead }),
                credentials: 'include'
            });
            ...
            if (response.ok) {
                ...
                if (data.success) {
                    ...
                    console.error('[MarkAsRead] Failed:', data.error);
                    setReadingProgress(previousProgress);
                }
            } else {
                console.error('[MarkAsRead] HTTP Error:', response.status);
                setReadingProgress(previousProgress);
            }
        } catch (e) {
            ...
            setMarkingRead(prev => ({ ...prev, [targetId]: false }));
        }
    };
    ...
    const isSectionRead = (sectionId) => {
        ...
        return (readingProgress?.readSections || []).some(
            section => {
                const sId = section.sectionId || section._id;
                return sId && sId.toString() === targetId;
            }
        ) || false;
    };
    ...
    const renderBlock = (block, index) => {
        ...
        if (block.type === 'link' || block.blockType === 'link') {
            ...
            if (url) {
                return (
                    <a href={url} target={target} rel={target === '_blank' ? 'noopener noreferrer' : undefined} className="text-[#C0934B] hover:underline font-bold my-4 block text-lg break-words max-w-full" onClick={(e) => e.stopPropagation()}>
                        {text || url}
                    </a>
                );
            }
        ...
        switch (bType) {
            ...
            case 'footer':
                // Optional: Implement if trial articles have footers. For now standard fallback.
                const footerData = (typeof parsedContent === 'object') ? parsedContent : {};
                return (
                    <div className="rounded-[15px] border-[0.5px] border-gray-300 overflow-hidden my-4 font-sans bg-white shadow-sm max-w-full">
                        <div className="bg-[#C0934B] py-4 flex justify-center gap-6 text-white text-lg">
                            <a href={footerData.facebook || '#'} className="hover:scale-110 transition-transform"><FaFacebookF /></a>
                            <a href={footerData.instagram || '#'} className="hover:scale-110 transition-transform"><FaInstagram /></a>
                            <a href={footerData.youtube || '#'} className="hover:scale-110 transition-transform"><FaYoutube /></a>
                        </div>
                        <div className="p-6 text-center text-black">
                            <p className="mb-2 text-sm text-gray-900 font-medium">Written By <span className="font-bold text-[#C0934B]">{(footerData.author === 'bharat Admin' || !footerData.author) ? 'Rupie Time' : footerData.author}</span></p>
                        </div>
                    </div>
                );
            default:
                return <div className="text-gray-800 mb-4 break-words max-w-full">{parseStyledText(block.content)}</div>;
        }
    };
        ...
        <div className="min-h-screen font-sans">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="mb-8 flex flex-col gap-4">
                    <Link href={`/user-dashboard/trial-products`} className="inline-flex items-center text-[#00301F] hover:underline font-medium w-fit">
                        <FaArrowLeft className="mr-2" /> Back To Trial Products
                    </Link>
                ...
                <div className="hidden lg:flex justify-end mb-4">
                    <button onClick={() => setIsDesktopSidebarOpen(!isDesktopSidebarOpen)} className="flex items-center gap-2 text-sm font-semibold text-[#00301F] hover:bg-gray-100 px-3 py-1.5 rounded-lg transition-colors">
                ...
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 relative transition-all duration-300 ease-in-out">
                    ...
                    <div id="article-print-content" className={`w-full transition-all duration-300 ease-in-out ${isDesktopSidebarOpen ? 'lg:w-[60%]' : 'lg:w-full'}`}>
                        ...
                        <h1 className="text-3xl md:text-4xl font-bold text-black leading-tight mb-4">{article.mainHeading}</h1>
                        ...
                        <div className="flex justify-between items-center text-sm text-gray-500 mb-8 border-b border-gray-200 pb-4">
                            ...
                            <div className="flex gap-4 items-center">
                                {article.author && <span>Published By <span className="text-black font-bold">{article.author}</span></span>}
                                <button onClick={handleFavoriteToggle} className="p-2 focus:outline-none transition-transform active:scale-95 ml-2">
                                    {isStarred ? <FaStar className="w-6 h-6 text-[#C0934B]" /> : <FaRegStar className="w-6 h-6 text-[#C0934B]" />}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-8">
                            {[...sections]
                                ...
                                .map((section) => {
                                    ...
                                    return (
                                        <div key={idx} className="border-[0.5px] border-gray-300 rounded-[20px] p-6 mb-8 relative bg-white/10 backdrop-blur max-w-full">
                                            {section.heading && <h3 className="text-xl font-bold mb-4">{section.heading}</h3>}
                                            <div className="space-y-6">
                                                {(section.contentBlocks || []).map((block, bi) => (
                                                    <div key={bi}>{renderBlock(block, bi)}</div>
                                                ))}
                                            </div>
                        ...
                        <div className="mt-8 mb-8 flex justify-end print:hidden">
                            <button onClick={handlePrint} className="flex items-center gap-2 bg-[#C0934B] text-white px-6 py-3 rounded-lg hover:bg-[#a87f3d] transition-colors font-bold">
                    ...
                    <div className={`
                     ...
                     `}>
                        <div className="h-full overflow-y-auto p-6 lg:p-0 lg:pl-8">
                            <h2 className="text-2xl font-bold mb-6 hidden lg:block">Trial Articles</h2>

                            {/* --- Date Filter Section --- */}
                            <div className="mb-6 p-4 bg-white/50 rounded-xl border border-gray-200 shadow-sm backdrop-blur sticky top-0 z-10">
                                <h3 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2"><FaInbox className="text-[#C0934B]" /> Filter By Article Create Date</h3>
                                    ...
                                    {(() => {
                                        ...
                                        return Object.entries(grouped).map(([header, articles]) => (
                                            <div key={header} className="mb-6">
                                                ...
                                                <div className="space-y-4">
                                                    {articles.map((item) => (
                                                        <div key={item._id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow bg-white/50 cursor-pointer">
                                                            <div className="flex justify-between items-start mb-2">
                                                                <div className="flex items-center gap-2">
                                                                    <Image src={sharpIcon} alt="" width={16} height={16} />
                                                            ...
                                                            <div className="flex justify-between items-center mt-2">
                                                                ...
                                                                <Link href={`${basePath}/${paramProductId}/articles/${item._id}`} className="text-xs font-bold text-[#00301F] flex items-center hover:underline">
                    ...
                    {/* Mobile Toggle */}
                    <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="fixed bottom-6 right-6 z-[60] bg-[#00301F] text-white p-4 rounded-full shadow-lg lg:hidden">
            ...
            <style jsx global>{`
                @media print {
                     ...
                     #article-print-content, #article-print-content * { display: block !important; visibility: visible !important; }
                     #article-print-content { position: absolute; left: 0; top: 0; width: 100%; }
                }
             `}</style>
        </div>
    );
